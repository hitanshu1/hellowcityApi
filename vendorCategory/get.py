...
from vendorCategory.schema import VendorCategorySchema


@AccessToken.token_required

async def getVendorCategory(request:web_request.Request, db_pool:aioodbc.Pool):
    try:   
        id =request.query.get('id')
        if id is not None:
            query = f"SELECT * FROM vendorCategory WHERE id = {id}"
            async with db_pool.acquire() as cnxn:
                    async with cnxn.cursor() as cursor:
                        ...
                        if row:
                            ...
                            vendorCategorySchema = VendorCategorySchema()
                            try:
                                vendor_data = vendorCategorySchema.load(item_dic)
                                serialized_data = vendorCategorySchema.dump(vendor_data)
            ...
            query = f"SELECT * FROM vendorCategory ORDER BY id OFFSET {Offset} ROWS FETCH NEXT {Limit} ROWS ONLY"
