...
from stuff.offer.schema import StuffSchema


@AccessToken.token_required
async def getStuff(request:web_request.Request, db_pool:aioodbc.Pool):
    try:   
        id =request.query.get('id')
        if id is not None:
            query = f"SELECT * FROM stuff WHERE id = {id}"
            async with db_pool.acquire() as cnxn:
                async with cnxn.cursor() as cursor:
                    ...
                    if row:
                        ...
                        try:
                            user_data = stuffSchema.load(item_dic)
                            serialized_data = stuffSchema.dump(user_data)
            ...
            query = f"SELECT * FROM stuff ORDER BY id OFFSET {Offset} ROWS FETCH NEXT {Limit} ROWS ONLY"
