...
async def getOffer(request:web_request.Request, db_pool:aioodbc.Pool):
    try:   
        id =request.query.get('id')
        if id is not None:
            query = f"SELECT * FROM offer WHERE id = {id}"
            async with db_pool.acquire() as cnxn:
                async with cnxn.cursor() as cursor:
                    ...
                    if row:
                        ...
                        try:
                            user_data = offer_schema.load(item_dic)
                            serialized_data = offer_schema.dump(user_data)
            ...
            query = f"SELECT * FROM offer ORDER BY id OFFSET {Offset} ROWS FETCH NEXT {Limit} ROWS ONLY"
