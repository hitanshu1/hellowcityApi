...
async def getUsers(request, db_pool:aioodbc.Pool):
    try: 
        ...
        if id is not None:
            ...
            async with db_pool.acquire() as cnxn:
                    async with cnxn.cursor() as cursor:
                        ...
                        if row:
                            ...
                            try:
                                user_data = user_schema.load(item_dic)
            ...
            query = f"SELECT * FROM users ORDER BY id OFFSET {Offset} ROWS FETCH NEXT {Limit} ROWS ONLY"
            async with db_pool.acquire() as cnxn:
                async with cnxn.cursor() as cursor:
                    await cursor.execute(query)
