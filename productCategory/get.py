...
from productCategory.schema import ProductCategorySchema
from vendor.schema import VendorSchema


@AccessToken.token_required

async def getProductCategory(request:web_request.Request, db_pool:aioodbc.Pool):
    try:   
        id =request.query.get('id')
        if id is not None:
            query = f"SELECT * FROM productCategory WHERE id = {id}"
            async with db_pool.acquire() as cnxn:
                    async with cnxn.cursor() as cursor:
                        ...
                        if row:
                            ...
                            try:
                                vendor_data = productCategorySchema.load(item_dic)
                                serialized_data = productCategorySchema.dump(vendor_data)
            ...
            query = f"SELECT * FROM productCategory ORDER BY id OFFSET {Offset} ROWS FETCH NEXT {Limit} ROWS ONLY"
