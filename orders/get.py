...
async def getOrders(request, db_pool:aioodbc.Pool):
    try: 
        ...
        if id is not None:
            ...
            async with db_pool.acquire() as cnxn:
                    async with cnxn.cursor() as cursor:
                        ...
                        if row:
                            ...
                            try:
                                user_data = order_schema.load(item_dic)
                                serialized_data = order_schema.dump(user_data)
            ...
            query = f"SELECT * FROM orders ORDER BY id OFFSET {Offset} ROWS FETCH NEXT {Limit} ROWS ONLY"
            async with db_pool.acquire() as cnxn:
                async with cnxn.cursor() as cursor:
                    await cursor.execute(query)
